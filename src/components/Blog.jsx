...
const mapPost = (post) => ({
  ...
  comments: post.comments?.map((comment) => ({
    ...comment,
    author: comment.profiles?.username || comment.profiles?.display_name || 'Reader',
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
    <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl dark:bg-gray-900">
      ...
      <div className="mb-5 flex items-center justify-center gap-3">
        {['facebook', 'google', 'github'].map((provider) => <button key={provider} type="button" disabled={loading} onClick={() => signInWithProvider(provider)} aria-label={`Continue with ${provider}`} className={`flex h-16 w-16 items-center justify-center rounded-full transition-transform hover:scale-105 disabled:cursor-not-allowed disabled:opacity-50 ${provider === 'facebook' ? 'bg-[#1877f2] text-white' : provider === 'google' ? 'border border-gray-200 bg-gradient-to-br from-white via-gray-50 to-gray-200 text-black shadow-sm dark:border-gray-700 dark:from-gray-800 dark:via-gray-900 dark:to-black' : 'bg-[#24292f] text-white'}`}>{providerIcons[provider]}</button>)}
      </div>
...
function CommentItem({ comment, comments, onReply, depth = 0 }) {
  const replies = comments.filter((item) => item.parent_id === comment.id);
  return <div className={depth ? 'relative ml-5 border-l-2 border-green-500/30 pl-5 sm:ml-10' : ''}>
    {depth > 0 && <span className="absolute -left-5 top-6 h-px w-5 bg-green-500/30" aria-hidden="true" />}
    ...
    {replies.length > 0 && <div className="mt-4 space-y-4">{replies.map((reply) => <CommentItem key={reply.id} comment={reply} comments={comments} onReply={onReply} depth={depth + 1} />)}</div>}
  </div>;
}
...
function PostDetail({ post, posts, user, onBack, onUpdate, onLike, onBookmark, onComment, onRequireSignup }) {
  ...
  const share = async () => {
    const postUrl = `${window.location.origin}${window.location.pathname}?blog=${encodeURIComponent(post.id)}`;
    if (navigator.share) await navigator.share({ title: post.title, text: post.excerpt, url: postUrl });
    else await navigator.clipboard?.writeText(postUrl);
  };
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto max-w-5xl px-4 pb-20 pt-8 md:px-8 md:pt-12">
    <button onClick={onBack} className="mb-8 flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-green-500"><ArrowLeft size={17} /> Back to all posts</button>
...
export default function Blog({ onClose, initialPostId = null }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data, error } = await supabase.from('posts').select('*, categories(name), post_tags(tags(name)), profiles!posts_author_id_fkey(display_name), comments(id, body, created_at, parent_id, profiles(username, display_name, avatar_url))').order('published_at', { ascending: false });
  ...
  const onComment = async (postId, body, parentId) => { if (!requireUser('signup')) return; const { data, error } = await supabase.from('comments').insert({ post_id: postId, author_id: user.id, body, parent_id: parentId || null }).select('id, body, created_at, parent_id, profiles(username, display_name, avatar_url)').single(); if (!error) { const next = { ...selected, comments: [...(selected.comments || []), { ...data, author: data.profiles?.username || data.profiles?.display_name || user.email, avatar: data.profiles?.avatar_url || profile?.avatar_url || null }] }; updatePost(next); setSelected(next); } };
  ...
  if (selected) return <div className="min-h-screen bg-white text-gray-950 dark:bg-black dark:text-white"><PostDetail post={selected} posts={posts} user={user} onBack={() => { setSelected(null); const url = new URL(window.location.href); url.searchParams.delete('blog'); window.history.replaceState({}, document.title, url.pathname + url.search); }} onUpdate={(next) => { updatePost(next); setSelected(next); }} onLike={onLike} onBookmark={onBookmark} onComment={onComment} onRequireSignup={() => requireUser('signup')} />{loginOpen && <LoginModal initialMode={loginMode} onClose={() => setLoginOpen(false)} onLogin={() => setLoginOpen(false)} />}</div>;
