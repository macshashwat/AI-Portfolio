...
import {
  ArrowLeft, Bookmark, Check, ChevronDown, Eye, Heart, Image, LockKeyhole,
...
const seedPosts = [
  {
    ...
    title: 'Shipping AI features that people can actually trust',
    excerpt: 'A practical field guide to building AI products with clear boundaries, useful feedback loops, and delightful UX.',
    content: 'The best AI experiences do not hide uncertainty. They make it legible, give people control, and improve with every interaction. Start with one narrow workflow, measure the outcome that matters, and only then add more intelligence.\n\nIn production, reliability is a product feature. Use structured outputs, validate every model response, and design graceful fallbacks before launch. Your users should always know what happened and what they can do next.',
    ...
    status: 'published',
    comments: [{ id: 1, author: 'Aarav', body: 'The structured-output point is so important.', replies: [] }]
  },
  {
    id: 'calm-backend',
    title: 'The calm backend: patterns for systems that scale',
    excerpt: 'How a few boring, observable primitives can make ambitious products faster to build and easier to operate.',
    content: 'Scale is rarely a single database decision. It is the result of boundaries that keep change affordable. Keep your domain logic independent, make jobs idempotent, and let the system tell you when it is under pressure.\n\nI prefer boring infrastructure with excellent dashboards. It gives a team room to focus on customer value instead of debugging surprises at 2 a.m.',
    category: 'Backend',
    tags: ['Architecture', 'Node.js', 'Observability'],
  ...
  },
  {
    ...
    title: 'Designing developer tools for momentum',
    excerpt: 'Small interaction details compound into a product that feels fast, focused, and made for its users.',
    content: 'Developer tools are used in a state of flow. Every unnecessary decision costs more than a click: it costs context. Good tools make the next useful action obvious, preserve intent, and keep feedback close to the action that caused it.',
...
function LoginModal({ onClose, onLogin }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
    <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl dark:bg-gray-900">
      <div className="mb-8 flex items-start justify-between"><div><p className="mb-2 text-sm font-semibold uppercase tracking-[0.2em] text-green-500">Admin access</p><h2 className="text-3xl font-bold">Welcome back</h2></div><button type="button" onClick={onClose} aria-label="Close"><X /></button></div>
      ...
      <label className="mb-2 block text-sm font-medium">Password<input type="password" className="mt-2 w-full rounded-xl border bg-transparent px-4 py-3 outline-none focus:ring-2 focus:ring-green-500" value={password} onChange={(e) => setPassword(e.target.value)} /></label>
      <p className="mb-6 text-xs text-gray-500">Use an account provisioned in Supabase Auth. Admin access is controlled by the database role.</p>
      {error && <p className="mb-4 text-sm text-red-500">{error}</p>}<Button disabled={loading} className="w-full bg-green-500 py-6 hover:bg-green-600"><LockKeyhole size={17} className="mr-2" /> {loading ? 'Signing in…' : 'Sign in securely'}</Button>
    </form>
  </div>;
}

function UserMenu({ user, profile, admin, onNewPost, onDashboard, onProfile, onSignOut }) {
  const [open, setOpen] = useState(false);
  return <div className="relative">
    <button onClick={() => setOpen(!open)} className="flex items-center gap-2 rounded-full border bg-white px-3 py-2 text-sm font-semibold shadow-sm dark:bg-gray-900" aria-expanded={open}>
    ...
    {open && <div className="absolute right-0 top-full z-20 mt-2 w-56 rounded-2xl border bg-white p-2 shadow-xl dark:bg-gray-900">
      ...
      <button onClick={() => { onProfile(); setOpen(false); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800"><UserCircle size={16} /> Edit profile</button>
      {admin && <><button onClick={() => { onNewPost(); setOpen(false); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800"><Plus size={16} /> New post</button><button onClick={() => { onDashboard(); setOpen(false); }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800"><Pencil size={16} /> Dashboard</button></>}
      <button onClick={onSignOut} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-red-500 hover:bg-red-50 dark:hover:bg-red-950"><LogOut size={16} /> Sign out</button>
    </div>}
  </div>;
}
...
function PostDetail({ post, posts, onBack, onUpdate, onLike, onBookmark, onComment }) {
  ...
  const addComment = () => { if (!comment.trim()) return; onComment(post.id, comment.trim(), replyTo); setComment(''); setReplyTo(null); };
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mx-auto max-w-5xl px-4 pb-20 pt-28 md:px-8">
    ...
    <div className="mb-10 flex flex-wrap items-center gap-3 border-y py-4"><button onClick={() => onLike(post)} className="flex items-center gap-2 rounded-full border px-4 py-2 text-sm hover:border-green-500 hover:text-green-500"><Heart size={17} /> {post.likes}</button><button onClick={share} className="flex items-center gap-2 rounded-full border px-4 py-2 text-sm hover:border-green-500 hover:text-green-500"><Share2 size={17} /> Share</button><button onClick={() => onBookmark(post)} className={`flex items-center gap-2 rounded-full border px-4 py-2 text-sm hover:border-green-500 hover:text-green-500 ${post.bookmarked ? 'border-green-500 text-green-500' : ''}`}><Bookmark size={17} /> {post.bookmarked ? 'Saved' : 'Save'}</button><span className="ml-auto flex items-center gap-2 text-sm text-gray-500"><Eye size={17} /> {post.views.toLocaleString()} reads</span></div>
...
function Dashboard({ posts, onNew, onEdit, onDelete, onOpen }) {
  return <main className="mx-auto max-w-6xl px-4 py-12 md:px-8">
    <div className="mb-10 flex items-end justify-between"><div><p className="mb-2 text-sm font-semibold uppercase tracking-[0.2em] text-green-500">Workspace</p><h1 className="text-4xl font-black">Content dashboard</h1></div><Button onClick={onNew} className="bg-green-500 text-black hover:bg-green-400"><Plus size={17} className="mr-2" /> New post</Button></div>
    ...
    <div className="overflow-hidden rounded-2xl border bg-white dark:bg-gray-900"><div className="grid grid-cols-[1fr_auto_auto] gap-4 border-b px-5 py-4 text-xs font-semibold uppercase tracking-widest text-gray-400"><span>Post</span><span>Status</span><span>Actions</span></div>{posts.map((post) => <div key={post.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 border-b px-5 py-5 last:border-0"><button onClick={() => onOpen(post)} className="text-left"><p className="font-bold hover:text-green-500">{post.title}</p><p className="mt-1 text-xs text-gray-500">{formatDate(post.publishedAt)} · {post.views} reads</p></button><span className="rounded-full bg-green-500/10 px-3 py-1 text-xs text-green-600">{post.status}</span><div className="flex gap-2"><button onClick={() => onEdit(post)} aria-label={`Edit ${post.title}`} className="rounded-lg p-2 hover:bg-gray-100 dark:hover:bg-gray-800"><Pencil size={17} /></button><button onClick={() => onDelete(post.id)} aria-label={`Delete ${post.title}`} className="rounded-lg p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-950"><Trash2 size={17} /></button></div></div>)}</div>
  </main>;
}
...
export default function Blog({ onClose }) {
  ...
  const filtered = useMemo(() => posts.filter((post) => post.status === 'published' && (category === 'All' || post.category === category) && `${post.title} ${post.excerpt} ${post.tags.join(' ')}`.toLowerCase().includes(query.toLowerCase())), [posts, category, query]);
  ...
  const uploadImage = async (file) => {
    if (!user) { setLoginOpen(true); return null; }
    if (file.size > 8 * 1024 * 1024) { setLoadError('Images and GIFs must be 8 MB or smaller.'); return null; }
    const extension = file.name.split('.').pop()?.toLowerCase() || 'bin';
  ...
  const requireUser = () => { if (!user) { setLoginOpen(true); return false; } return true; };
  const onLike = async (post) => { if (!requireUser()) return; const { error } = await supabase.from('post_likes').upsert({ post_id: post.id, user_id: user.id }); if (!error) { const next = { ...post, likes: post.likes + 1 }; updatePost(next); setSelected(next); await supabase.from('posts').update({ likes: next.likes }).eq('id', post.id); } };
  ...
  const onComment = async (postId, body, parentId) => { if (!requireUser()) return; const { data, error } = await supabase.from('comments').insert({ post_id: postId, author_id: user.id, body, parent_id: parentId || null }).select('id, body, created_at, profiles(display_name)').single(); if (!error) { const next = { ...selected, comments: [...(selected.comments || []), { ...data, author: data.profiles?.display_name || user.email }] }; updatePost(next); setSelected(next); } };
  if (selected) return <div className="min-h-screen bg-white text-gray-950 dark:bg-black dark:text-white"><PostDetail post={selected} posts={posts} onBack={() => setSelected(null)} onUpdate={(next) => { updatePost(next); setSelected(next); }} onLike={onLike} onBookmark={onBookmark} onComment={onComment} /></div>;
