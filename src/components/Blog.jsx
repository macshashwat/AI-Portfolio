...
function Dashboard({ posts, onNew, onEdit, onDelete, onOpen }) {
  return <main className="mx-auto max-w-6xl px-4 py-12 md:px-8">
    ...
    <div className="overflow-hidden rounded-2xl border bg-white dark:bg-gray-900"><div className="grid grid-cols-[1fr_120px_96px] gap-4 border-b px-5 py-4 text-xs font-semibold uppercase tracking-widest text-gray-400"><span>Post</span><span className="text-center">Status</span><span className="text-right">Actions</span></div>{posts.map((post) => <div key={post.id} className="grid grid-cols-[1fr_120px_96px] items-center gap-4 border-b px-5 py-5 last:border-0"><button onClick={() => onOpen(post)} className="text-left"><p className="font-bold hover:text-green-500">{post.title}</p><p className="mt-1 text-xs text-gray-500">{formatDate(post.publishedAt)} · {post.views} reads</p></button><span className="justify-self-center rounded-full bg-green-500/10 px-3 py-1 text-xs capitalize text-green-600">{post.status === 'published' ? 'Published' : 'Draft'}</span><div className="flex gap-2"><button onClick={() => onEdit(post)} aria-label={`Edit ${post.title}`} className="rounded-lg p-2 hover:bg-gray-100 dark:hover:bg-gray-800"><Pencil size={17} /></button><button onClick={() => onDelete(post.id)} aria-label={`Delete ${post.title}`} className="rounded-lg p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-950"><Trash2 size={17} /></button></div></div>)}</div>
  </main>;
}
